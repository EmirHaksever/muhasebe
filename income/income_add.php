...
$pageTitle = "Gelir Ekleme";

$sorgu = $pdo->prepare("SELECT * FROM income_categories WHERE is_active = 1 ORDER BY name ASC");
$sorgu->execute();
$kategoriler = $sorgu->fetchAll(PDO::FETCH_ASSOC);

$sorgu2 = $pdo->prepare("SELECT * FROM projects WHERE is_active = 1 ORDER BY name ASC");
$sorgu2->execute();
$projeler = $sorgu2->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($error === '') {
        ...
        header('Location: income_add.php?ok=1');
        exit;
    }
}
?>
    ...
    <div class="col-12 col-lg-12">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-0 pt-4 pb-0">
                <h2 class="h4 mb-1">Gelir Ekle</h2>
                <p class="text-muted mb-0">Yeni gelir kaydı oluşturun.</p>
            </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="project_id" id="project_id" class="form-select">
                            ...
                            <?php foreach ($projeler as $proje): ?>
                                <option value="<?php echo (int)$proje['id']; ?>"
                                    <?php echo ((string)($_POST['project_id'] ?? '') === (string)$proje['id']) ? 'selected' : ''; ?>>
